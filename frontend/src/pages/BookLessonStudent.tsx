...
import { Lesson, LessonRequestStatus, LessonStatus } from "@/types/lesson";
...
export default function BookLessonStudent() {
	...
	const sendLessonRequest = (lessonRequest: Lesson) => {
	...
	const avaliableLessons = teacherLessons.filter(
		(lesson) =>
			...
			!studentRequests.find((request) => request.lesson.id === lesson.id)
	);
					...
					<CardHeader className="flex items-center">
						...
						<Link to={`/teacher/${id}`}>
							...
							{teacherLessons &&
								...
								teacherLessons[0].teacher.fullName}
						</Link>
					</CardHeader>
