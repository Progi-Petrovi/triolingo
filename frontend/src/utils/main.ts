...
export const lessonAggregatetoLesson = async (
	...
	return {
		...
		title: lessonAggregate.lesson.language + " lesson " + lessonAggregate.lesson.id,
		language: lessonAggregate.lesson.language,
		teacher,
		teacherPayment: lessonAggregate.lesson.teacherPayment,
		status: LessonStatus[lessonAggregate.lesson.status as keyof typeof LessonStatus],
	};
};
...
export const lessonBulkAggregateToLessons = async (
	...
	for (const lessonDTO of lessonBulkAggregate.lessons) {
		...
		lessons.push(await lessonAggregatetoLesson({ lesson: lessonDTO, teacher }));
	}
	return lessons;
};
...
export function enumToStringList(enumObject: Record<string, string>) {
	return Object.keys(enumObject).map((key) => enumObjectToString(enumObject, key));
}
